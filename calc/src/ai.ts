...
const trappingMoveNames: string[] = ['Whirlpool', 'Fire Spin', 'Sand Tomb', 'Magma Storm',
  'Infestation', 'Wrap', 'Bind'];
...
const twoHitMoves: string[] = ['Bonemerang', 'Double Hit', 'Double Iron Bash', 'Double Kick',
  'Dragon Darts',
...
const zeroBPButNotStatus: string[] = ['(No Move)', 'Electro Ball', 'Metal Burst', 'Endeavor',
  'Bide',
  'Seismic Toss', 'Punishment', 'Flail', 'Reversal', 'Gyro Ball', 'Magnitude', 'Heat Crash',
  'Heavy Slam', 'Present', 'Natural Gift', 'Beat Up', 'Fissure', 'Guillotine', 'Horn Drill',
  'Super Fang',
  'Low Kick', 'Sheer Cold', 'Final Gambit', 'Mirror Coat', "Nature's Madness", 'Psywave',
  'Night Shade', 'Dragon Rage',
...
const offensiveSetup: string[] = [
  ...
  'Growth',
// removing tail glow, nasty plot and work up because they are offensive setup but they have their
// own section
];
// TODO: test tail glow
...
function getTripleAxelDamage(res: Result) {
  const tripleAxelDamageRolls: number[][] = [];
  let tripleAxelDamage: number[] = [];

  let i = 0;
  for (const bp of [20, 40, 60]) {
    const move: Move = res.move.clone();
    move.bp = bp;
    move.hits = 1;
...
function getAIDeadAfterShellSmash(res: any[], playerMaxDamage: number) {
  ...
  let playerMaxDamageAfterSS = 0;
  const defender = playerMoves[0].defender.clone();
...
function computeDistribution(array: number[]): { [key: number]: number } {
  ...
  const totalCount = sortedArray.length; // Total number of elements in the array

  sortedArray.forEach(value => {
    if (!(value in distribution)) {
      // Store distribution value
      distribution[value] = sortedArray.filter(v => v === value).length / totalCount;
    }
  });
...
function setKeyStrings(keyString: string, splitKeys: string[]): string[] {
  ...
  // SUSPECTED BUG (behaviour preserved deliberately): this was `for (const
  // splitKey in splitKeys)`, which iterates an array's INDICES ('0', '1', ...)
  // rather than its values, so the test below asks whether the key string
  // contains the character '0'/'1'/... instead of the actual split key. It very
  // likely wants `of`. Changing it would move real AI probabilities, so it needs
  // a maintainer's call plus an expected-output test before being switched.
  for (let i = 0; i < splitKeys.length; i++) {
    const splitKey = String(i);
...
function splitKeyString(keyString: string, subString: string): string[] {
  ...
  // SUSPECTED BUG (behaviour preserved deliberately): this was `for (const index
  // in indices)`, so `index` is a position WITHIN `indices` rather than the
  // part index stored there — it rewrites parts 0..n-1 instead of the collected
  // positions. That is almost certainly also why the filter above had to be
  // "inversed" (see the comment there): two mistakes that partly cancel. Left
  // as-is because untangling them changes AI output; needs a maintainer's call.
  for (let i = 0; i < indices.length; i++) {
    const newKeyString = [...parts];
    newKeyString[i] = newKeyString[i].replace(subString, '0');
...
function updateProbabilityWithVariance(probabilities: KVP[], key: string, prob: number) {
  ...
  for (const combination of combinations) {
    ...
    const newKeyValues = newKey.split('/');
    for (let i = 0; i < newKeyValues.length; i++) {
      const newKeyValue = newKeyValues[i];
...
function getAiDeadToSecondaryDamage(result: any) {
  ...
  const damageTaken = statusDamage + weatherDamage;
...
function shouldAIRecover(aiMon: Pokemon, recoveryPercentage: number,
  ...
  if (aiFaster) {
    ...
    const playerCanKillAIAfterRecovery = playerMaxRoll >= Math.min(aiMonCurrentHP + aiRecoveredHP,
      aiMonMaxHP);
...
function isSuperEffective(move: Move, monTypes: I.TypeName[], gravity = false, ringTarget = false) {
  const type1Effectiveness = getMoveEffectiveness(move.gen, move, monTypes[0], false, gravity,
    ringTarget);
...
// returns a new list of KVP's, [{key: "Move1:0/Move2:6/Move3:0/Move4:0", value: 0.63}, {key:
// "Move1:0/Move2:6/Move3:8/Move4:0", value: 0.37}]
function updateMoveKVPWithMoveStrings(moveKVPs: KVP[],
  moveStringToAdd: { move: string; score: number; rate: number }): KVP[] {
  // populate moveStringsToAdd to every score from that array to every instance of that move in
  // moveKVPs
  const newKvps: KVP[] = []; // [{key: "Move1:0/Move2:6/Move3:0/Move4:0", value: 1}]
...
function calculateHighestDamage(moves: any[]): KVP[] {
  ...
  const p1CurrentHealth = moves[0].defender.curHP();

  // console.log(moves); // DEBUG

  // Damaging Trapping Moves should always come back as -1 damage
  // TODO: use this for later if you need to iterate on other things, but for now this isn't
  // nessesary
  /*
    let newMoves: any[] = [];
  ...
  // TODO: consider using above code to turn off crits except for cases where Crit should be turned
  // on

  // console.log(moves); // DEBUG

  const arrays = moves.map(move =>
    move.damageRolls().map((roll: number) => Math.min(p1CurrentHealth, roll)));
  const aiFaster = moves[0].attacker.stats.spe >= moves[0].defender.stats.spe;

  // list of damage distributions for the move
  const moveDistributions = arrays.map(array => computeDistribution(array));

  // calculate the probability distribution of which dict will have the highest key
  const probabilities: KVP[] = [];
  ...
  const allChoices =
    cartesian(moveDistributions.map(distribution => objectEntriesIntKeys(distribution)));

  // console.log(allChoices); // debug

  for (const choice of allChoices) {
    const keys = choice.map(([key, value]) => key);
    const moveProbabilities: number[] = choice.map(([key, value]) => Number(value));

    const keysForMaximumCheck = [1];
    let i = 0;
    for (const key of keys) {
      if (moves[i].move.category === 'Status' ||
            isNamed(moves[i].move.name, 'Explosion', 'Final Gambit', 'Rollout', 'Misty Explosion',
              'Self-Destruct', 'Relic Song', 'Meteor Beam', 'Future Sight', 'Counter',
              'Mirror Coat') ||
    ...
    const maximumKey = Math.max(...keysForMaximumCheck);
    ...
    for (const key of keys) {
      ...
      if (key >= p1CurrentHealth) {
        ...
        if (isNamed(moves[i].move.name, 'Relic Song', 'Meteor Beam', 'Future Sight') ||
          isTrapping(moves[i].move)) {
  ...
  const probabilitiesWithVariance: KVP[] = [];
  ...
  const critBoostMoveDist: KVP[] = [];

  for (const damagingMoveDistKVP of probabilitiesWithVariance) {
    const moveArr = damagingMoveDistKVP.key.split('/');
    const moveStringsToAdd: { move: string; score: number; rate: number}[] = [];
    ...
    moveArr.forEach((moveScoreString, index) => {
      ...
      if (isHighCritRate(move.name) &&
        isSuperEffective(move, moves[index].defender.types, moves[0].field.isGravity,
          moves[index].defender.item === 'Ring Target')) {
 ...
 */
export function generateMoveDist(damageResults: any[], fastestSide: string,
  aiOptions: {[key: string]: boolean }): number[] {
  // DEBUG
  ...
  const finalDist: number[] = [];
  moves.forEach((move, i) => {
    finalDist[i] = 0.0;
  });

  // handle multi-hit moves
  // not making this into a function because passing moves to a function will mess up the
  // functionality
  ...
  const damagingMoveDist = calculateHighestDamage(moves);
  ...
  const playerHealthPercentage =
    Math.trunc((moves[0].defender.originalCurHP / moves[0].defender.stats.hp) * 100);
  const aiHealthPercentage =
    Math.trunc((moves[0].attacker.originalCurHP / moves[0].attacker.stats.hp) * 100);
  ...
  const aiSlowerButFasterAfterPara = !aiFaster &&
    moves[0].attacker.stats.spe > Math.trunc(moves[0].defender.stats.spe / 4);
  const trickRoomUp = moves[0].field.isTrickRoom;
  const playerLeechSeeded = moves[0].field.defenderSide.isSeeded;
  const aiHasAnyStatRaised =
    Object.values(moves[0].attacker.boosts).some(value => (value as number) > 0);
  const weather = moves[0].field.weather;
  const playerHasStatusMove = playerMoves.some(x => getMoveIsStatus(x.move.name, x.move.bp));
  // AI has at least 1 status move
  const aiHasStatusMove = moves.some(x => getMoveIsStatus(x.move.name, x.move.bp));
  ...
  for (const damagingMoveDistKVP of damagingMoveDist) {
    ...
    moveArr.forEach((moveScoreString, index) => {
      ...
      const moveScore = Number(moveScoreString.split(':')[1]);
      ...
      // TODO: This function can probably use continues, so I probably should do that for
      // performance lol
      ...
      const moveHasPriority = move.priority > 0 || (moveName === 'Grassy Glide' &&
        terrain === 'Grassy');
      ...
      const isDamagingSpeedReducing = moveName === 'Icy Wind' || moveName === 'Electroweb' ||
        moveName === 'Rock Tomb' ||
            moveName === 'Mud Shot' || moveName === 'Low Sweep' || moveName === 'Bulldoze' ||
              moveName === 'Scary Face';
      if (isDamagingSpeedReducing && moveScore === 0 && anyValidDamageRolls) {
        if (playerAbility !== 'Contrary' && playerAbility !== 'Clear Body' &&
          playerAbility !== 'White Smoke' && !aiFaster) {
          moveStringsToAdd.push({
            move: moveName,
            score: 6,
            rate: 1,
      ...
      if (isNamed(moveName, 'Skitter Smack', 'Trop Kick', 'Snarl', 'Mystical Fire',
        'Breaking Swipe') && moveScore === 0) {
        const affectedMoveType = moveName === 'Trop Kick' ||
          moveName === 'Breaking Swipe' ? 'Physical' : 'Special';
        if (playerAbility !== 'Contrary' && playerAbility !== 'Clear Body' &&
          playerAbility !== 'White Smoke' &&
                    playerMoves.findIndex(
                      (x: { move: { category: string } }) => x.move.category === affectedMoveType
                    ) !== -1) {
          moveStringsToAdd.push({
            move: moveName,
            score: 6,
      ...
      // Fling, Role Play, doubles weakness policy, magnitude, eq is just for doubles, so leave it
      // for now
      ...
      if (moveName === 'Light Screen' || moveName === 'Reflect') {
        ...
        if ((moveName === 'Light Screen' && aiLightScreen) ||
          ...
          const playerHasAnyOfCorrespondingSplit =
    playerMoves.some(x => x.move.category === correspondingMoveSplit &&
                        (x.move.bp > 0 || (zeroBPButNotStatus.includes(x.move.name) &&
                          x.move.name !== '(No Move)')));
    ...
    Math.trunc((moves[0].attacker.originalCurHP / moves[0].attacker.stats.hp) * 100);
      ...
      if (moveName === 'Thunder Wave' || moveName === 'Stun Spore' || moveName === 'Nuzzle' ||
        ...
        const paraIncentive = aiSlowerButFasterAfterPara || hexIndex !== -1 ||
          playerCharmedOrConfused;
      ...
      if (moveName === 'Will-O-Wisp') {
        ...
        if (playerHasStatusCond ||
          playerTypes.findIndex((type: string) => type === 'Fire') !== -1) {
          moveStringsToAdd.push({
            move: moveName,
            score: -20,
            rate: 1,
          ...
          const hexIndex = moves.findIndex(x => x.move.name === 'Hex'); // hehe inHEX more like
          const physicalIndex = damageResults[0].findIndex(
            (x: { move: { category: string; bp: number } }) =>
              x.move.category === 'Physical' && x.move.bp > 0
          );
      ...
      if (moveName === 'Yawn' || moveName === 'Dark Void' || moveName === 'Grass Whistle' ||
        moveName === 'Sing' || moveName === 'Hypnosis') {
        const sleepPreventingAbility = playerAbility === 'Insomnia' ||
          playerAbility === 'Vital Spirit' || playerAbility === 'Sweet Veil';
        if (sleepPreventingAbility || playerHasStatusCond || terrain === 'Electric' ||
          terrain === 'Misty') {
          moveStringsToAdd.push({
            move: moveName,
            score: -20,
          ...
          if (!aiSeesKill) {
            ...
            if ((dreamEaterIndex !== -1 || nightmareIndex !== -1) && (snoreIndex === -1 &&
              sleepTalkIndex === -1)) { sleepScore++; }
        ...
        if (playerHasStatusCond ||
          ...
          if (playerHealthPercentage > 20 && !aiSeesKill) {
            ...
            if (playerHighestRoll === 0 &&
                            (movesetHasMoves(moves, 'Hex', 'Venom Drench') ||
                              moves[0].ability === 'Merciless')) {
              toxScore += 2;
            }
      ...
      const isContrary = aiAbility === 'Contrary';
      ...
      if (isNamed(moveName, 'Power-Up Punch', 'Swords Dance', 'Howl',
        ...
        if (aiDeadToPlayer ||
                    ((moveName !== 'Power-Up Punch' && moveName !== 'Swords Dance' &&
                      moveName !== 'Howl') &&
      ...
      if (moveScore === 0 && isContrary) {
        ...
        if (isNamed(moveName, ...offensiveSetup)) {
          // stop Charge Beam and Power Up Punch from being chosen on ground/ghost types
          // respectively
      ...
      if (isNamed(moveName, 'Coil', 'Bulk Up', 'Quiver Dance', 'No Retreat', 'Calm Mind') ||
        ...
        if (isNamed(moveName, 'Coil', 'Bulk Up', 'No Retreat', 'Curse') || actAsBulkUp) {
          if (playerMoves.some(x => x.move.category === 'Physical' &&
            !getMoveIsStatus(x.move.name, x.move.bp)) &&
                        !playerMoves.some(x => x.move.category === 'Special' &&
                          !getMoveIsStatus(x.move.name, x.move.bp))) {
          ...
          if (playerMoves.some(x => x.move.category === 'Special' &&
            !getMoveIsStatus(x.move.name, x.move.bp)) &&
                        !playerMoves.some(x => x.move.category === 'Physical' &&
                          !getMoveIsStatus(x.move.name, x.move.bp))) {
            // console.log("is defensive setup"); // DEBUG
            isDefensiveSetup = true;
          } else {
      ...
      if (isDefensiveSetup) {
        ...
        if (boostsDefAndSpDef && (moves[0].attacker.boosts.def < 2 ||
          moves[0].attacker.boosts.spdef < 2)) {
          defensiveScore += 2;
        }
      ...
      if (moveName === 'Belly Drum') {
        const sitrusRecovery =
    aiItem === 'Sitrus Berry' ? Math.trunc(moves[0].attacker.stats.hp / 4) : 0;
        const hpAfterBellyDrum =
    moves[0].attacker.originalCurHP -
    Math.trunc(moves[0].attacker.stats.hp / 2) + sitrusRecovery;
        const aiNotDeadAfterBellyDrum = playerHighestRoll < hpAfterBellyDrum;
        if (aiMaxedOutAttack ||
          // useless move
          moves[0].attacker.originalCurHP - Math.trunc(moves[0].attacker.stats.hp / 2) <= 0) {
          moveStringsToAdd.push({
            move: moveName,
            score: -40,
            rate: 1,
      ...
      // I lost 2 mons to aiDeadToPlayer check here not be referenced in the docs, I should've
      // known that was weird
      if (moveName === 'Focus Energy' || moveName === 'Laser Focus') {
        const critIncentive = move.ability === 'Super Luck' || move.ability === 'Sniper' ||
                                        move.item === 'Scope Lens' ||
                                          movesetHasHighCritRatioMove(moves);
      ...
      // I'm sure we can multiply rate to figure it out, and move this above recovery moves so we
      // can funnel these moves to normal recovery
      // bool to flag if sun-based recovery should get handled like other healing moves
      let sunBasedHealingOverflow = false;
      ...
      if (moveName === 'Rest') {
        const restIncentive: number = aiItem === 'Lum Berry' || aiItem === 'Chesto Berry' ||
                                                ...
                                                moves[0].ability === 'Shed Skin' ||
                                                  moves[0].ability === 'Early Bird' ||
                                                (moves[0].ability === 'Hydration' &&
                                                  weather.includes('Rain')) ? 1 : 0;
      ...
      if (moveName === 'Taunt') {
        if (playerTaunted) {
          ...
          if ((movesetHasMove(playerMoves, 'Trick Room') && !trickRoomUp) ||
                        movesetHasMove(playerMoves, 'Defog') &&
                          moves[0].field.attackerSide.isAuroraVeil && aiFaster) {
            moveStringsToAdd.push({
              move: moveName,
              score: 9,
              rate: 1,
      ...
      if (moveName === 'Counter' || moveName === 'Mirror Coat') {
        ...
        const aiSturdyAndFullHP = (aiAbility === 'Sturdy' || aiItem === 'Focus Sash') &&
          aiHealthPercentage === 100;
        const correspondingMoveSplit = moveName === 'Counter' ? 'Physical' : 'Special';
        const playerOnlyHasMovesOfCorrespondingSplit =
    playerMoves.every(x => x.move.category === correspondingMoveSplit && x.move.bp > 0);
        const playerNoMovesOfCorrespondingSplit =
    playerMoves.every(x => x.move.category !== correspondingMoveSplit || x.move.bp === 0);
      ...
      if (moveName === 'Magnet Rise') {
        if (aiMagnetRisen) {
          ...
          let playerHasDamagingGroundMove = false;
          ...
          for (const groundMoveIndex of playerGroundMoveIndexes) {
            ...
            if ((typeof groundMoveDamage === 'number' && groundMoveDamage !== 0) ||
                            (Array.isArray(groundMoveDamage) &&
                              groundMoveDamage.reduce((a: number, b: number) => a + b, 0) > 0)) {
      ...
      const playerCanBeGrounded = playerTypes.includes('Flying') || playerAbility === 'Levitate';
